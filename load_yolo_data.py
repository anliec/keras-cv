...
def load_yolo_image(image_path: str, input_size):
    ...
    print("original shape {} resized in {}".format(im.shape, input_size))
...
def load_yolo_pair(path_to_image: str, input_size):
    path_to_annotation = os.path.splitext(path_to_image)[0] + ".txt"
    scores, sizes = load_yolo_gt(path_to_annotation, input_size)
    im = load_yolo_image(path_to_image, tuple(input_size))
    return im, scores, sizes


def load_yolo_pair_for_map(args):
    return load_yolo_pair(args[0], args[1])


def load_annotations_from_dir(dir_path: str, input_shape, images_regex: str = "*.jpg"):
    images = glob.glob(os.path.join(dir_path, images_regex))
    print("{} images found in {}".format(len(images), dir_path))
    x = np.zeros(shape=(len(images),) + tuple(input_shape) + (3,), dtype=np.float)
    y_score = np.zeros(shape=(len(images),) + tuple(input_shape), dtype=np.float)
    y_size = np.zeros(shape=(len(images),) + tuple(input_shape), dtype=np.float)

    pool = multiprocessing.Pool()

    dataloader = pool.imap_unordered(load_yolo_pair_for_map, zip(images, itertools.repeat(input_shape)), chunksize=25)
