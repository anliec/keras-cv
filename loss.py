'''
Based on the The Keras-compatible loss function for the SSD model.
Currently supports TensorFlow only.
...
class SSDLikeLoss:
    '''
    Based on the SSD loss, see https://arxiv.org/abs/1512.02325.
    '''
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        Arguments:
            y_true (array): A Numpy array of shape `(batch_size, #boxes, #classes + 12)`,
                ...
                box in `y_pred`. The last axis must have length `#classes` and contain
                `[classes one-hot encoded]`
                in this order, including the background class. Important: Boxes that
                you want the cost function to ignore need to have a one-hot
                class vector of all zeros.
            y_pred (Keras tensor): The model prediction. The shape is identical
                to that of `y_true`, i.e. `(batch_size, #boxes, #classes)`.
        ...
        def f2():
            ...
            # belong to the background class in the ground truth data. Note that this doesn't necessarily mean that the
            # model predicted the wrong class for those boxes, it just means that the loss for those boxes is the
            # highest.
