...
def get_last_conv_ancestor(model: tf.keras.Model, layer: tf.keras.layers.Layer):
    if type(layer) == tf.keras.layers.Add:
        prev_name = layer.input[1].name.split('/')[0]
    else:
...
def latex_table_model():
    ...
    config = {"size_value": [110, 200], "dropout_rate": 0.0, "dropout_strategy": "last",
              "layers_filters": (8, 8, 16, 16), "expansions": (1, 6, 6), "use_resnet": True}
    ...
    for l in model.layers[::-1]:
        if type(l) == tf.keras.layers.BatchNormalization:
            ...
            activation = "Softmax"
        elif type(l) == tf.keras.layers.Add:
            residual = "\\checkmark"
            ...
            lines.append("{} & ${}$ & ${}$ & ${}$ & {} & {} & {} & ${}$ & {} \\\\"
                         "".format(name, filters, kernel_size,
                                   strides, activation, bn, residual, l.input.shape[1:], in_name))
            bn = ""
            activation = "Linear"
            residual = ""
